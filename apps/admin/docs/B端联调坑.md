...
> **44 条**实战坑，覆盖 Batch 2 ~ Batch 7（认证/RBAC → 管理后台 → 题库 CRUD
> → 导入管道 → 导入向导 → 组卷引擎）。每条都按
...
>   **34~44** Batch 7 组卷引擎（前置修正 + 恢复接口 + Pass 2a 前端）。
...
1. **后端**：`sections` 一旦传入就整卷重建是**既定的正确语义**，不改；
   但已发布（且未开「允许发布后编辑」）时直接 `40901` 拒绝 —— 不能让已经考过的卷被改掉。
2. **前端**：
   - **基本信息编辑只发非 `sections` 字段**（`ExamBasicInfoDialog`），
     绝不把详情对象整个回传；
   - 改分段结构走**单独的入口**（`ExamSectionsDialog`），并做三重保护：
     说出"将清空现有 N 道题"、**必须勾选「我明白」**确认键才可用、已发布时入口直接禁用。

**教训（可以推广的一条）**：**接口的一个可选字段可能携带"删除"语义。**
设计这类"部分更新"接口时，破坏性字段要么**单独一个端点**，
要么**在参数名上就写明**（如 `replace_sections`），别让它和 `title` 长得一样。
前端侧的对策是死规矩：**提交前构造 payload，而不是把读到的对象原样回传**。
