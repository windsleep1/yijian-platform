...
**⚠️ 本坑未完全归因**：28 分钟这个量级，两条候选
（A：`wait-*` 真连不可达地址 / B：`_scratch_database` 的 `CREATE`/`DROP DATABASE` 阻塞）
**都解释不了**：

- B **已实测否证** —— 有 `idle in transaction` 旁观者时 `CREATE` 0.90s、`DROP ... WITH (FORCE)` 6.67s，**不阻塞**；
- A 最多到**分钟级**（上表），到不了 28 分钟。

要定位还差**该 run 的 pytest 步骤日志尾部**（job 日志要认证，匿名读不到；
step 级状态倒是能匿名从 job 页 HTML 里拿到 —— 见 `docs/18` §12.1）。

**同族**：坑 50（并行 Edit 互相覆盖）、坑 51（门禁）、坑 54（采集边界）、坑 55（skip 伪装绿）、坑 56（日志出处）
—— 全是**"没有任何东西会报错"**那一类。
