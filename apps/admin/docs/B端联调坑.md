...
> **68 条**实战坑，覆盖 Batch 2 ~ Batch 7（认证/RBAC → 管理后台 → 题库 CRUD
...
>   **51~53** 门禁整顿（验收脚本的"假绿" / 覆盖率门禁陷阱 / 结构化文件塞注释）｜
>   **54~55** 补测批（采集边界假象 / `pytest.skip` 静默）｜ **56** 证据的身份 ｜
>   **57** pytest 无超时 ｜ **58~60** Batch 8 统计看板（SQL 参数类型契约 / 环境问题伪装成缺陷 /
>   `next build` 沙箱拦截）｜ **61~62** Batch 9 审核链（JSONB 回来是 `str` /
>   沙箱删除预算按轮累计）｜ **63~64** C 端起手（本机 build 的**两种**失败形态 /
>   变异 harness 的"已还原"是假绿）｜ **65** 读 CI 状态（每次 push 两条 run 的干扰 + 用 jobs API 看每步）
>   ｜ **66** 诊断读不到 = 没有诊断（`-rs` 顶掉 `-rfE` / `tail` 打到 403 的 stdout /
>   `timeout_method=thread` 是 `os._exit` / **真因是 `run:` 默认带 `-e`**）
>   ｜ **67** BL-16 真身（替身按 `recv` 次数回包 + 产品无界等待）
>   ｜ **68** 多起一个 PG ⇒ 全量管道成片 50001。
...
## 61. `asyncpg` 没注册 jsonb codec：JSONB 列回来的是 **str** ★（2026-09-25）

写审核链的直连断言时踩到的。`conftest.sql_fetch` 用的是**裸** `asyncpg.connect(...)`，
没有 `set_type_codec("jsonb", ...)`，于是 `content_change_logs.diff` 与
`audit_logs.before_data/after_data` 这类 **JSONB 列取回来是 `str`，不是 `dict`**。

```
diff = rows[0]["diff"]                 # 实际是 '{"before": {...}, "after": {...}}'
diff["after"]["comment"]               # TypeError: string indices must be integers, not 'str'
```

### 为什么值得单列一条

**报错指向的是你的断言，不是取回值的形状。** 这句话是这条坑的全部要害：

- 报错行是 `assert diff["after"]["comment"] == "..."`，看起来像"**断言写错了**"；
- 于是第一反应会是去改断言（甚至改成 `diff["after"]...` 的另一种写法），
  而真正的事实是"**我拿到的不是我以为的那个东西**"。

⇒ **与坑 56（证据的身份）同族**：先问"我手上这个值是什么形状、从哪来的"，再谈它等于几。

**处置**：在测试里加一个 `_j()`（`str → json.loads`，其余原样）并**在 docstring 里写明为什么**，
因为下一个人写新的直连断言时**一定会再遇到它**。

> ⚠️ 更彻底的做法是给 `conftest.sql_fetch` 注册 jsonb codec，那样全仓的直连断言都受益。
> 本批**没有**这么做：它会同时改变已有断言拿到的类型（现在有些地方已经在 `json.loads`），
> 改动面比本批要大 —— 属于"顺手改基础设施"，**不塞进审核链这一批**。

## 62. 沙箱的"批量删除预算"是**按轮累计**的 —— 前一步的清场会把后面的步骤堵死（2026-09-25）

**现象**：本地跑完整管道，pytest 全绿，但最后一步 `coverage combine` 失败：

```
[safe-delete][SAFE_DELETE_BULK_CONFIRM_REQUIRED] {"count":55,"threshold":50,"scope":"turn",
    "targets":["...\.coverage.api"],"targetCount":1}
```

`targetCount` 只有 **1** 个文件 —— 单次操作完全无害，却因为 **`scope: turn` 的累计计数已达 55**
（阈值 50）被拦。

**真因**：同一条命令序列里，**我先手工删了 `htmlcov/`（4.7M，55 个文件额度）**，
于是把这一轮的删除配额提前用光了。`combine` 只是**撞上了配额的下一次检查**。

### 判据（写下来免得再查一遍）

- 沙箱的删除守卫是**按轮累计**的，**不是按单次操作**看数量 ——
  所以"这个操作只删 1 个文件，不可能被拦"是**错的直觉**；
- `SAFE_DELETE_BULK_CONFIRM_REQUIRED` 的 `count` 是**已经用掉的额度**，
  `targetCount` 才是**这次要删几个**。**两个数字要看对**（又一次"证据的身份"）。
- ⇒ 收尾清场（删 `htmlcov/`、删 `__pycache__`）**别放在管道前面**；
  要么放在最后，要么接受"这一轮的覆盖率报告跑不出来"。

**这不是源码问题**，与坑 60（`next build` 沙箱拦截）同类：**本机环境 != CI**。
CI 上 `combine` 一直正常（那里没有这个守卫）。

## 63. 本机 `next build` 有**两种**失败形态，且都在"编译完成之前" —— 别把它读成"代码有问题" ★（2026-09-26）

坑 60 记过"在本机跑不通"。2026-09-26 为了验证"跨包 TS 能不能被打出来"，
又跑了两遍，拿到**第二种**失败形态 —— 两种都值得记，因为它们的**误导方向不同**。

| 前置状态 | 失败形态 | 你看到的 |
|---|---|---|
| `.next` **存在**（73M） | `[safe-delete] 操作失败: spawnSync …genie-trash… ETIMEDOUT` | 像"沙箱删不掉旧产物" |
| `.next` **不存在** | 走到了 `Creating an optimized production build ...`（webpack 已经在跑），然后 `uncaughtException [Error: EPERM: operation not permitted, open '…\.next\trace']` | 像"构建过程本身失败" |

### 判据（重要：这决定"能不能从本地 build 失败里得出结论"）

- **两种都发生在编译完成之前** ⇒ 本机 build 失败**既不能推出"代码有问题"，也不能用来验跨包解析**。
  它的信息量是 **0**（除了"本机跑不了"这件事本身）。
- ⇒ **"跨包 TS 能否被 Next 打出来"这个问题，本地无解**，只能由 CI 回答 ——
  这正是把 `next build` 放进 CI 的理由（BL-12，`docs/22` §9.2.3 选 A）。
- ⚠️ 所以：**看到本机 build 红，不要开始改代码。** 先去 CI 看同一步。
  （与坑 54/55/57 同族：**报错指向的地方不是失败点**。）

**同族**：坑 60（沙箱拦截）/ 坑 62（删除配额）。

## 64. 变异 harness 的"已还原"是**假绿** —— Python 文本模式把 LF 写成了 CRLF ★（2026-09-26）

**现象**：`tools/local-verify/mutate-api-core.py` 跑完 6 个变异，汇总行写
**"仓库已还原：是"**，6/6 捕获 —— 一切正常。**但紧接着 `npm run format:check:shared` 红了**：

```
[warn] ../../packages/api-core/test/auth-chain.test.ts
[warn] Code style issues found in the above file.
```

实测行尾：`auth-chain.ts` **157 个 CRLF**、`auth-chain.test.ts` **239 个 CRLF**（prettier 配置要求 `lf`）。

**根因（两层，缺一个都不会翻车）**：

```
① Python 的 write_text / read_text 是**文本模式**：在 Windows 上写出 `\r\n`
② 但**读**的时候 universal newlines 又把 `\r\n` 变回 `\n`
⇒ `read_text() == original` 这个"还原成功"的比较，**根本看不见行尾被改过**
```

⇒ 于是 harness 的判据一直在比较"**语义等价**"，而不是"**文件没变**"。
**它是假绿**：报告绿、仓库脏。

**判据 / 解法**：

- **还原与校验一律按字节**：`read_bytes()` / `write_bytes()`，比较也用 `==` 比字节。
  写文本别用 `write_text`（除非显式 `newline="\n"`）。
- **⚠️ 这条规则 MEMORY 里本来就有**（"用 Python 写仓库内文件要 `newline='\n'`"），
  仍然踩了 —— 与 `.ps1` BOM 那次**完全同型**：**"工具默认行为"级的坑，不是"没想到"级的坑**。
  知道 ≠ 会避免；**只有把它变成"按字节"这种机械动作才拦得住**。
- 抓出它的不是 harness（它自己说绿），而是**格式门禁** —— 又一次印证：
  **多一道不同性质的检查，才可能抓住另一道的假绿**。

**同族**：坑 51（验收脚本的假绿）/ 坑 55（`pytest.skip` 静默）/ 坑 56（证据的身份）——
全部属于"**没有任何东西会报错**"那一类。

## 65. 每次 push 有**两条** run：`pages build` 的 success 不是 CI 的结论 ★（2026-09-26）

**现象**：用 `GET /repos/…/actions/runs` 看状态，每次 push 都返回**两条**：

```
CI                                    push     ≈13 分钟   failure   ← 门禁
pages build and deployment            dynamic  ≈40~60 秒  success   ← GitHub Pages 自动流程
```

**只看"最近一条 conclusion=success"就会得出"CI 是绿的"** —— 而那条 success 是
**GitHub Pages 的自动部署**，跟门禁毫无关系。**它长期混在列表里**（本项目从 09-16 起就有）。

### 正确的读法（三步，都匿名可用）

| 步 | 做什么 | 说明 |
|---|---|---|
| 1 | **按 workflow 名过滤** | `/actions/workflows/{id}/runs`（本仓 `CI` 的 id = 362493984）比全量列表干净得多，**不会把 pages 混进来** |
| 2 | **拿每一步的 `conclusion` 与起止时间** | `GET /actions/runs/{id}/jobs` —— **匿名可读**，逐 step 给 `conclusion` / `started_at` / `completed_at` |
| 3 | 日志内容 | ⚠️ **匿名读不到（403）** ⇒ 只能靠"哪一步 + 跑了多久"定位，再决定要不要人工取日志 |

### 判据：什么时候"时长"可以当定位依据

```
pytest 步骤：02:40:06 → 02:52:18 = 12 分 12 秒
workflow 里：timeout-minutes: 12
⇒ 撞超时（不是断言失败）—— **下一步「覆盖率门禁」恒为 skipped** 也印证了这一点
```

⚠️ 这不是"拿时长当结论"（那条禁的是"用时长去否定 `conclusion`"）。
这里的用法是：**结论已经是 failure，时长用来回答"失败在哪一步、是哪种失败"**。
两个数字（配置里的 12 / 实测的 12:12）**吻合**才是证据；不吻合就得另找。

### 一个**匿名可读**的诊断通道（2026-09-26 实测，BL-16 靠它推进）

`GET /repos/{owner}/{repo}/check-runs/{job_id}/annotations` —— **返回 200**（不需要 token）。
于是可以在 CI 步骤里把关键行回显成 `::error::`，再从这条通道读回来：

```yaml
grep -ohE 'tests[\\/][A-Za-z0-9_]+\.py", line [0-9]+, in [A-Za-z0-9_]+' "$LOG" | while IFS= read -r l; do
  echo "::error title=挂起位置::$l"
done
```

⇒ **"日志读不到"不等于"诊断不了"** —— 换一条通道，把"要的信息"主动推出去。

⚠️ **坑：WebFetch 有 15 分钟缓存**。用**同一个 URL** 复查 CI 状态会拿到**旧快照**
（实测：连续两次读同一个 `runs/{id}/jobs`，第二次返回的还是"in_progress"，
差点据此得出"它还在跑"的错误结论）。⇒ **复查时加一个扰动参数**（`?b=<时间戳>`）。
这与坑 56（证据的身份）同族：**先问"这份数据是什么时候取的"**。

**同族**：坑 56（证据的身份 —— 先问"这条 run 属于哪个 workflow"）。

## 66. **诊断读不到 = 没有诊断** —— 三个坑叠在一起，让"哪条用例挂了"沉默了两次 ★（2026-09-26）

> 这一段是给**排障工具自己**写的。它的价值不在"某条用例挂了"，而在
> **"你为排障写的东西，将来一定会在你最需要它的那一刻不说话"**。

**现象**：BL-16 的定位手段上线后，CI 的 `pytest` 步从 **12m12s 降到 121s**
（说明 `pytest-timeout` 确实生效了），但 **`GET /check-runs/{id}/annotations` 里
一条 `::error::` 都没有** —— 只有 GitHub 自己的三条（Node 20 弃用、exit code 1、Ubuntu 迁移）。
也就是：**机制生效了，诊断没生效**。

**三个原因，每个单独都不致命，叠起来就是完全的沉默**：

| # | 我写的 | 实际后果 | 实测证据 |
|---|---|---|---|
| 1 | `-q -rs` | **`-r` 是"替换"不是"追加"**：`-rs` 把默认的 `-rfE` 顶掉 ⇒ 短汇总里只剩 skip，**`FAILED` 行整段不出现** | 本机同一条失败用例：`-q -rs` 输出里**没有** "short test summary info" 这一段；换成 `-q -rfEXs` 才有 `FAILED tests/...::test_x - AssertionError` |
| 2 | 三条 grep 全按"挂起"写（`Stack of MainThread` / `tests/x.py", line N, in test_y`） | 那次 CI 是**普通失败**（没有堆栈）⇒ **一条都不匹配** | annotation 列表里没有任何 `Stack of` 相关项 ⇒ 反过来证明**这次根本没触发超时** |
| 3 | 兜底 `tail -15 "$LOG"` | 写到了 **stdout** —— 而 **job 日志匿名 403**，等于写进了保险箱 | `GET /actions/jobs/{id}/logs` → `403 Must have admin rights` |

**顺带验清的一件事（也是必须的）**：`timeout_method = thread` 触发时走的是
**`os._exit(1)`** —— 本机实测：被打断后**后续用例不再执行、连汇总行都没有**
（`-q -rs` 与 `-q -rfEXs` 两种命令行输出**逐字相同**）。
⇒ 缓冲区里没 flush 的输出会**整体丢失**，于是诊断看到的不是"错误内容"而是"日志是空的"。
⇒ 必须 `PYTHONUNBUFFERED: "1"`。（`signal` 方法能让整轮继续跑完，但 **Windows 没有 SIGALRM**
⇒ 本地会退回 thread，"本地/CI 同源"就没了；而 thread 已经能打出**卡在哪一行**的堆栈，
所以保留 thread、只补缓冲与通道。）

**解法（两条，缺一不可）**：

1. **诊断要有"自身可读性"判据**：头部**无条件**发一条 annotation，带
   `exit / log_bytes / log_lines / verdict`。
   它**自己能分辨"日志是空的"和"grep 没匹配上"** —— 光这一条就能把三种失败形态一次分清。
   再加一条 ★**原始尾部兜底**（`tail -25` 逐行进 annotation），以及第二通道
   `$GITHUB_STEP_SUMMARY`（长文本比逐行 annotation 合适，两条通道互为备份）。
2. **落到结构上**：诊断段用 `<<< PYTEST-DIAG:START/END >>>` 标记，
   `tools/local-verify/test-pytest-diag.sh` **原地提取**它（不存在"测试里的逻辑 vs 脚本里的逻辑"
   这第二份副本），拿**三种真实日志形态**（普通失败 / 挂起堆栈 / **空日志**）+ **1 个变异**
   去验，并**接进 CI**（`CI 诊断段自检` 步，放在最前面）。
   > ★ 这个 harness **第一次跑就抓出它自己一个 bug**：判据写成
   > `$(has "$OUT" "X" && echo 0 || echo 1)`，而 `has` 的退出码是它内部最后一条
   > `echo` 的（永远 0）⇒ `&&` 总成立、替换结果是**两行**，判据永远不成立。
   > 也就是**"变异存活"的第四种解释：判据本身写坏了**（前三种见硬约定 J）。

> ⚠️ **同一段 harness 当天第二次翻车 —— 这次是我"为了说明它"而写的那句话把它弄坏的**：
> 我在 CI 的另一处注释里**写出了标记原文**，于是提取器**把那条注释也当成了标记**，
> 从那行开始往下截，抓到 **235 行**垃圾；再加上去缩进用的是 `cut -c13-`（**按字节切**），
> 直接把中文切成了乱码。最终表现：**进程被 SIGTERM、一个字都不输出** —— 与"脚本写坏了"完全同形。
> ⇒ 两条修法都落到结构上（不靠"记得"）：① 标记匹配用**完整形态**（带尖括号），
> 并在 harness 里**断言 START 只出现一次**；② 去缩进用 `sed 's/^ \{12\}//'` 匹配空格，
> **绝不用 `cut -c`**（它按字节）。★ 顺带把 harness 里**所有删除动作去掉** ——
> 本机沙箱的删除预算是**按轮累计**的（坑 62），一个 `trap … rm -rf` 会让它跑几次之后开始 SIGTERM。
> **同族**：坑 65（两条 run 分不清）—— 都是"**获取器抓错了对象，而且失败时不说话**"。

> ⚠️⚠️ **真正的真因（2026-09-26 第三轮才定位，且本机可决定性复现）**：
> 上面那三个原因（`-rs` / grep 只覆盖"挂起" / `tail` 打到 stdout）**都是真的、也都该修**，
> 但**没有一个是"annotation 一条都没发出来"的原因**。真正的原因是：
>
> **GitHub Actions 的 `run:` 默认用 `bash -e {0}` 执行 —— `-e` 本来就开着**，
> 它不是靠脚本里写 `set -e` 才打开的。⇒ 那条"pytest 会失败"的管道一旦非零，
> `-e` **立刻结束整个脚本**，诊断段**根本没机会跑**（连 `rc=${PIPESTATUS[0]}` 都到不了）。
>
> **本机决定性对照**（同一段脚本，只换 shell 选项）：
>
> ```
> bash -eo pipefail ./step_fail.sh   →   输出为空（诊断 0 行）   ← 与 CI 现象完全一致
> bash              ./step_fail.sh   →   诊断正常（verdict= / ::error 都有）
> ```
>
> ⚠️ 而**旧注释里明明写着"`set -e` 故意不加：加了会在管道那一步直接退出"** ——
> 说明作者（我）对**自己的工具链**有一个**错误的信念**。这类错的杀伤力在于：
> 它让你在**错误的方向上**连续找两轮替罪羊，而每一次"发现"看起来都像真的。
>
> **三处修法**（都落到结构上）：
> ① `run:` 的**第一行**写 `set +e`；
> ② annotation **显式带 `file=` / `line=`** —— GitHub 自己的失败标注都带 `path` + 行号，
>    而"不带 file 的 job 级 annotation 在 `/check-runs/{id}/annotations` 里读不到"是**另一个必须消掉的可能**；
>    **两个原因都消掉**，下次才分得清"没跑"和"跑了但看不见"；
> ③ harness 新增 **[6] 组**：把**整段步骤脚本**抽出来、把 `coverage run` 换成 `false`，
>    用 **`bash -eo pipefail`**（GitHub 的真实形态）跑，断言诊断**确实输出**；
>    并**带证伪** —— 把 `set +e` 那行删掉后必须**变哑**，否则这条检查就不是检查（硬约定 J）。
>
> ⚠️ **前两轮的 harness 为什么抓不到它**：它用**普通 `bash`（没有 `-e`）**跑**诊断段** ——
> 又一个"两个口径"。⇒ 教训：**提取式 harness 必须用与 CI 相同的 shell 选项，而且要跑"整段"，
> 而不是"你想测的那一段"。**
> ★ 附带第四个教训：`[0c]` 自检第一次是**抓到了它自己的说明文字**（注释里写着那个反模式的名字）
> ⇒ 检测器必须**只扫代码行**。同族：一切"判据命中了自己"的假红。

> **同族（本条自己也在里面）**：坑 51 / 55 / 56 / 65 —— 以及
> **"诊断读不到"这条坑的真因，恰恰是"诊断根本没跑"**，而我一度以为它跑了。

**判据（可自检的那一句）**：*把 job 日志设成不可读，仅凭 annotation 还能不能说清"发生了什么"？*
不能 = **没有诊断**，别把它当成"排障工具就绪"。

**同族**：坑 51（验收脚本的假绿）/ 55（`pytest.skip` 静默）/ 56（证据的身份）/
65（两条 run 分不清）—— 全部属于"**没有任何东西会报错**"那一类；
以及硬约定 H：**读不到的判据等于没有判据**。

## 67. **BL-16 的真身**：替身"按 `recv` 次数回包" + 产品"无界等待" —— 两个小缺陷叠加成 12 分钟挂起 ★（2026-09-26）

CI 连红 **12 次**、每次都撞 12 分钟整步超时，真身就是这条。

**定位路径**（只有诊断修好之后才可能）：annotation 直接给出
`verdict=hang` + `挂起位置 = tests/test_cli_commands.py:339 test_wait_redis_returns_0_when_it_answers`。

**两个缺陷，缺一个都不会挂这么久**：

| # | 所在 | 缺陷 | 单独后果 |
|---|---|---|---|
| **B** | **测试替身** `_PongServer` | "一次 `recv` 回一次包"（正确做法是**按命令条数**回）。redis-py 建连时先发两条 `CLIENT SETINFO` 握手命令，而这两条**经常合进同一个 TCP 段** ⇒ 替身只回一次 ⇒ 客户端在等**第二个握手应答**、连 `PING` 都还没发出去 | 用例**间歇性**死等（本机 12 次里 2 次；CI 上几乎每次 ⇒ 12/12 连红） |
| **A** | **产品** `app/cli.py::_wait_redis` | `from_url` 既没 `socket_connect_timeout` 也没 `socket_timeout` ⇒ 那个 `deadline` **只守得住重试循环、守不住单次 `ping()`** —— 一次 ping 可以无限等，循环再也回不到 deadline 判断 | `--timeout 10` 形同虚设；`python -m app.cli wait-redis` 在生产里**能永久挂住** |

⇒ **B 是触发器，A 是放大器**：没有 A，B 只是"偶尔慢一下"；没有 B，A 永远不被触发。
**"替身不完整"和"产品不设上界"都是欠债，它们会在同一处复合。**

**怎么当场证明 B（而不是继续猜）**：写了个"进度标记 + 服务器侧抓包"的复现器 ——
`from_url / ping / aclose` 三步各打标记、每步跑在**独立子进程**里（那次卡死不能污染后面的迭代）、
5s 上限杀进程，并把**替身收到的原始字节**打出来。挂起那一次的服务器侧只有**一个** TCP 段：

```
RECV b'...CLIENT...SETINFO...LIB-NAME...redis-py... *4\r\n$6\r\nCLIENT\r\n$7\r\nSETINFO\r\n$7\r\nLIB-VER...'
SENT b'+OK\r\n'          ← 两条命令，只回了一次
```

**修法（四条，都落到结构上）**：

1. **产品**：`from_url(..., socket_connect_timeout=2, socket_timeout=2)`，并写明"deadline 只守循环"；
2. **替身**：实现最小 RESP 切分（`_commands`），**按命令条数**回包；半条命令留给下次 `recv` 拼；
3. **两条机械守卫**（都可证伪）：`test_wait_redis_bounds_each_attempt`（拦住 `from_url` 少了超时 ——
   **删掉那两个参数即红**，已实测）、`test_pong_server_replies_once_per_command`
   （主动把两条命令写进**同一个** `sendall` 造出合包，断言收到**两条**应答）；
4. 原用例补**耗世上界**断言（就绪路径 `< 2s`）—— 让"再次挂起"表现为**失败**，而不是**挂住整轮**。

**验收**：修前同一条用例连跑 40 次 **8 次挂死**（16.9s 被杀）；修后 **40/40 通过，最慢 2.69s**。

⚠️ **顺手记一个反模式**：隔壁 `_wait_db` 是 `asyncpg.connect(..., timeout=5)` —— **有界**；
`_wait_redis` 没有，而**某个测试还在自己的替身里绕过它**（mock 掉 `from_url`）。
⇒ **"症状在测试里消失了，缺陷还在产品代码里"** —— 与坑 66 同族，也与"在接缝上打补丁"同族。

**同族**：坑 57（无界等待）/ 坑 66（诊断读不到）/ 硬约定 N。

## 68. 本地全量管道那 82 个失败，根因是"**我多起了一个 PG**" ★（2026-09-26）

现象：全量管道 `82 failed / 205 passed / 34 skipped`；失败**全在登录链路**
（`test_admin_v7`、`conftest` 的超管 fixture），跳过原因写着"超管登录失败（50001）：请先执行 seed-admin"。
而管道日志自己显示 `seed-rbac` / `seed-admin` **都是 exit=0**。

**根因**：`yb-pipe.py` **自己会起 PG、末尾自己 `pg_ctl stop`**；
而我为了"保险"，在跑它之前**又手动起了一个 postmaster**（同一个数据目录、同一个端口）。
两个 postmaster 抢同一份 data 目录 ⇒ API 中途连不上 PG
（API 日志尾部：`ConnectionRefusedError` × 99 + `WinError` × 107）⇒ 成片 50001。

**判据**：*跑之前问一句"这个脚本自己管不管它的依赖进程？"* —— 管，就别替它管。
**同族**：坑 57 的"两个口径"；以及"**多加一层保险反而制造了故障**"。

**怎么快速定位它**（值得记）：失败**不是均匀分布**，而是"前 205 个通过、之后成片失败" ⇒
**中途有东西死了**，不是"代码错了"（代码错了会从头就错）⇒ 先看 **API 日志尾部的错误类型统计**。
...
### 8. 扫待办清单 —— 看有没有"触发条件已经成立"的 ★

- [ ] 打开 `docs/21-待办清单.md`，**从上到下读"触发条件"那一列**，逐条问"**它现在成立了吗？**"
- [ ] 成立 → **本批处理**（或明确改期，并把新理由写回该文件）
- [ ] 不成立 → 跳过；但**顺手看一眼"状态"列有没有过期**
      （例：某个 ⚪ 未触发的前置其实已经上线了）

**为什么单列一节**：待办清单的前两条（编号 / 触发条件）只保证它"**写得清楚**"，
但**没有任何交付流程会因为"当时说过要回来看看"而停下来** ——
这一点和 §5 的覆盖率棘轮是**同一个道理**（门槛写死就退化成"一个数字"）。
所以第 3 个条件"检查点"必须是个**机械动作**，也就是本节。

⚠️ **扫描时要连"状态列是否过期"一起看**，不只是"触发没触发"：
本项目已出过这一类 —— `MEMORY.md` 里的进程状态停在"门槛 93.10"，
而当时实际已经抬到 **94.40**、S2 也已交付。**没人报错，只是数字变旧了。**

> 规则正文（三条件 / 反例 / "以后想起来"不是待办）见 `docs/21-待办清单.md` §1，
> 已升格为硬约定 **P**。

### 9. 扫「文档声称」清单 —— 凡是"声称"而非"验证"的都要有下落 ★

- [ ] 打开 `docs/23-文档与现实对照清单.md`，看有没有**本批新产生的声称**
      （本批写过的文档里，凡是**陈述语气**说"有 X / 已是 X"的句子）
- [ ] 有 → **当场验**（写清命令），或列进清单并注明"未验证"
- [ ] 顺手看一眼清单里**已列条目**有没有被本批顺手修掉的 → 改状态

**为什么单列一节**：它和 §5（覆盖率假红）、§8（待办过期）是**同一族** ——
**"文档说 A、现实是 B"不会报错，只在你依赖它的那一刻失效**（没有 `X-Admin-Token`，
后端就真的没有那道二次校验；`docs/03` 写着表结构，但它只是设计稿）。

⚠️ **判据要能定位到一条命令**。例：
"`docs/04` 说管理端要带 `X-Admin-Token`" → `grep -rn "X-Admin-Token"`
→ 只命中 `docs/` ⇒ **未实现**。**"我觉得应该有"不算验证。**

> 规则与首轮清单见 `docs/23`；本节的落点是**每批收尾**（与 §8 同一个检查点时刻）。
