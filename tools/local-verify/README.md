...
> `services` 只有 49.6% —— 注意这是**已有代码**里没被测试打到的部分
> （错误分支 / 边界处理 / 辅助方法），**不是"未来批次的代码"**；补测现在就能做。
> `(顶层)` 13.5%（`cli.py` / `main.py` 只有启动路径被跑到）。
>
> **抬门槛的触发条件**（写在同一份 `.coveragerc` 里）：
> `实测 ≥ 70%` 或 `services ≥ 60%` → 把 `fail_under` 改成 71。
