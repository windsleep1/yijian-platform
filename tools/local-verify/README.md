...
>
> 已在 Windows + PostgreSQL 16.15 + Python 3.13 上实测通过：**`86 passed, 1 skipped`**（Batch 7 时点）。
...
===================== 86 passed, 1 skipped in 31.49s =====================
```

> 前面还会有 `test_admin_v3.py`（8）、`test_admin_v4.py`（12）、`test_admin_v5.py`（14）、
> `test_admin_v7.py`（32）、`test_idgen.py`（15）。
> **当前全量是 `86 passed, 1 skipped`** ——
