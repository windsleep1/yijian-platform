...
## 11. Pass 2b：组卷规则 + 新建试卷（已交付）

### 11.1 交付物

| 文件 | 说明 |
|---|---|
| `app/(console)/paper-rules/page.tsx` | 规则列表 + 行级**启用/停用**与**删除** |
| `app/(console)/exams/new/page.tsx` | 新建试卷（**手动选题** / **规则自动组卷** 两种模式） |
| `components/PaperRuleDialog.tsx` | 规则新建/编辑（**实时试算** + 缺口二次确认） |
| `components/RuleEditor.tsx` | 抽题约束编辑器（逐字段校验 + 配比提示） |
| `components/RulePreviewPanel.tsx` | 试算结果面板（两页共用） |

**配套的后端补口**（47 → 48 接口）：`POST /admin/paper-rules/preview` —— dry-run 试算。
spec 要的"保存规则前预览这条规则能抽到多少题"与"自动组卷实时展示抽题结果"
在此之前**没有任何接口能支撑**（`paper_rules` 只有 list/create/update/delete）。

**关键实现：抽题逻辑抽成 `_draw_rule()` 供两边共用。**

```
compose_exam  ─┐
               ├─→ _draw_rule(db, subject_id, rule, rng, excluded) → (抽到的行, 每档候选数)
preview_paper_rule ─┘
```

这是这个预览**唯一有意义的做法**：试算说"能抽满"而真组卷报缺口，
用户会彻底不信它。共用同一个函数，`need/got/missing` 与 `shortfalls` 天然同源。
用例 `test_rule_preview_matches_actual_compose` 直接拿两边结果逐项对比来钉住这一点。

### 11.2 实时试算（dry-run）

规则一改就重算（600ms 防抖），结果就地展示：

- **结论**：`题库充足：N 道都能抽满` / `题库不足：需要 N 道，只能抽到 M 道，还差 K 道`
- **逐条规则**：计划 / 能抽到 / 缺口
- **为什么抽不到**：把放宽阶梯每档的候选数摊开（`exact=0` → 放宽后也只有 12 道）。
...
| 模式 | 做什么 | 建完去哪 |
|---|---|---|
| **手动选题** | 自己定卷面结构（分段表），建空卷 | 跳详情并**自动打开「加题」面板**（走 `#add`） |
| **规则自动组卷** | 选已保存规则或临时配一套，**建之前先试算** | 跳详情看组卷结果与缺口 |

规则模式的落库顺序是**先建空卷 → 再 `apply_sections=true` 组卷** ——
这样分段里的"计划题数"天然等于规则要的题数，抽不满时缺口才说得清。

手动模式跳转用 **hash（`#add`）而不是 `?add=1`**：App Router 下
`useSearchParams()` 在客户端组件里需要 Suspense 边界，hash 读 `window.location` 没这个约束。

### 11.4 行级反馈的**两种结局**（`keepRow`）

| 操作 | 结局 | `keepRow` |
|---|---|---|
| 删除规则 | 这一行该走了（硬删除） | `false` → 标记后延时移除 |
| 启用 / 停用 | 这一行**还在**，只是换了状态 | `true` → 只标记，不移除 |

这正是 `useRowActionFeedback` 需要 `keepRow` 的原因：早期只有"移除"一种结局，
拿它做"停用"会把**还在列表里的行错误地隐掉**，用户以为被删了。

### 11.5 验收对照

| 验收标准 | 结果 | 证据 |
|---|---|---|
| 一条规则生成完整卷 | ✅ | `12`：31 题 / 39 分、分段「已排满」、校验通过可发布 |
| 故意配题库不足 → shortfalls 准确显示 | ✅ | `13`：`需要 200 · 抽到 0 · 缺 200`、整段标红 |
| **规则保存前 dry-run 能预警"当前题库不足"** | ✅ | `03`（弹窗内实时）+ `11`（建卷页预警块） |
| 两种创建模式切换清晰 | ✅ | `08`：卡片式单选 + 「当前」标识；`09` 手动 / `10` 规则 |
| 规则编辑字段校验（题型配比合理、题数 >0） | ✅ | 逐字段红字；跨规则"同一题池会互相挤占"提示 |
| 复用 2a 的就地反馈 + 行标记 | ✅ | `05`（停用，行留下）/ `07`（删除，行移除） |
| 截图 ≥10 张 | ✅ | **13 张**（`apps/admin/docs/screenshots/batch7-pass2b/`，md5 无重复） |
| `run-smoke.ps1` 全绿 | ✅ | 见 §5.6 |

E2E：`tools/local-verify/seed-pass2b.py` + `e2e-pass2b.py`。
