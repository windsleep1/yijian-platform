...
>
> 当前后端共 **43 个接口**，pytest 全量 **86 passed, 1 skipped**。
...
│   │   ├─ requirements.txt
│   │   ├─ tests/                     # pytest（86 passed, 1 skipped）：smoke + v3 + v4 + v5 + v7 + idgen
│   │   └─ app/
...
# 全量 86 passed, 1 skipped（1 skipped 是 6000 行导入用例，需环境变量显式开启）
```
...
- **Batch 7 Pass 2（下一步）** → 试卷管理前端：`/exams` 列表（含"显示已归档"开关）、
  ...
  后端 **14 个接口**与 `shortfalls` / 版本锁定 / 归档恢复 / viewer 只读能力已就绪（见 `docs/13` §9）
