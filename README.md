...
>
> 当前后端共 **48 个接口**，pytest 全量 **107 passed, 1 skipped**。
...
│   │   ├─ requirements.txt
│   │   ├─ tests/                     # pytest（107 passed, 1 skipped）：smoke + v3 + v4 + v5 + v7 + idgen
│   │   └─ app/
...
# 全量 107 passed, 1 skipped（1 skipped 是 6000 行导入用例，需环境变量显式开启）
```
...
- **Batch 7 Pass 2（下一步）** → 试卷管理前端：`/exams` 列表（含"显示已归档"开关）、
  ...
  后端 **19 个接口**与 `shortfalls` / 版本锁定 / 加题移题 / 归档恢复 / 结构防护 / 规则试算 / viewer 只读能力已就绪（见 `docs/13` §9–§11）
